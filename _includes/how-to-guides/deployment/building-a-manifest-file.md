...
Use a manifest file to configure and define your HAProxy load balancer deployed by Cloud 66. These changes will be either be applied when you redeploy a stack with more than one server, rebuild HAProxy or edit [HAProxy CustomConfig](/{{ include.product }}/addins/haproxy.html).
...
Environment variables set in your manifest file will only apply during the initial build of your stack. Please refer to our documentation on [environment variables](/{{ include.product }}/tutorials/env-vars.html) if you'd like to set them beyond this point.
...
[Background processes](/rails/deployment/proc-files.html) can be deployed and managed by Cloud 66. Any process in a `Procfile` will be picked up, deployed and monitored by the system.
...
For more information about **LiveLogs** and additional examples, please see the [LiveLogs help page]({{ include.product }}/how-to-guides/deployment/shells/setting-up-custom-livelogs.html).
