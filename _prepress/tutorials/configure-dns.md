...
order: 1
lead: "How to configure your DNS with Prepress"
legacy: false
tags: ["customization"]
...
Cloud 66 creates unique [DNS hostnames](/{{page.collection}}/references/server-ip-addresses.html#cloud-66-hostnames) for each server you deploy with us. These use animal names to make them more easily recognizable. For example:

```shell
puma.prepress-demo.development.c66.me
...
In addition to unique hostnames per server, Cloud 66 also offers [Failover Groups](/{{page.collection}}/tutorials/failover-groups.html) - managed, quick-response DNS addresses that automatically follow the web endpoints of applications.
...
3.  ​Use [network redirects](/{{page.collection}}/tutorials/service-network-configuration.html) to permanently redirect all traffic from `example.com` to `www.example.com`.
...
* Learn how to set up a [Failover Group](/{{page.collection}}/tutorials/failover-groups.html)
* Learn how to [configure network access](/{{page.collection}}/tutorials/service-network-configuration.html) to your application
